<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center p-5 relative overflow-hidden">
    <!-- 粒子背景 -->
    <div class="particles absolute inset-0 pointer-events-none" ref="particlesContainer"></div>
    <!-- 背景装饰 -->
    <div class="absolute inset-0 pointer-events-none">
      <div class="floating-shape absolute w-32 h-32 bg-blue-100/30 rounded-full top-[10%] left-[10%] animate-float-1"></div>
      <div class="floating-shape absolute w-40 h-40 bg-indigo-100/30 rounded-full top-[70%] right-[10%] animate-float-2"></div>
      <div class="floating-shape absolute w-24 h-24 bg-sky-100/30 rounded-full top-[20%] right-[20%] animate-float-3"></div>
      <div class="floating-shape absolute w-36 h-36 bg-blue-200/30 rounded-full bottom-[20%] left-[20%] animate-float-4"></div>
    </div>
    <!-- 主容器 -->
    <div class="relative z-10">
      <div class="auth-card flex w-[900px] h-[600px] bg-white/20 backdrop-blur-xl border border-white/30 rounded-3xl shadow-2xl overflow-hidden relative">
        <!-- 登录表单 -->
        <div
          class="w-1/2 flex flex-col items-center justify-center px-10 z-10 transition-all duration-700 ease-in-out"
          :class="isRegisterMode ? 'translate-x-4 opacity-100 scale-100' : 'translate-x-0 opacity-60 scale-95'"
        >
          <div class="text-center mb-8">
            <h1 class="text-4xl font-bold mb-3">
              <span class="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">KnoWeb</span>
            </h1>
          ...
          <form class="space-y-5 w-full max-w-sm" @submit.prevent="handleLogin">
            ...
            <div class="space-y-4">
              <div class="relative">
                ...
                />
                <span class="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none">
                  <!-- 优化后的邮箱svg -->
                  <svg width="22" height="22" fill="none" viewBox="0 0 24 24" class="opacity-70">
                    <rect width="20" height="14" x="2" y="5" rx="3" stroke="#3b82f6" stroke-width="2"/>
                    <polyline points="3,7 12,13 21,7" fill="none" stroke="#6366f1" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </span>
              </div>
              <div class="relative">
                ...
                <span class="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none">
                  <!-- 优化后的密码svg -->
                  <svg width="22" height="22" fill="none" viewBox="0 0 24 24" class="opacity-70">
                    <rect x="5" y="11" width="14" height="8" rx="3" stroke="#6366f1" stroke-width="2"/>
            ...
            <div class="flex items-center">
              <label class="flex items-center cursor-pointer text-slate-600 text-sm">
                <input
                  ...
                  class="sr-only"
                />
                <div class="w-4 h-4 mr-2 border-2 border-slate-300 rounded flex items-center justify-center transition-all duration-200"
                     :class="{ 'bg-blue-500 border-blue-500': form.remember }">
                  <svg v-if="form.remember" class="w-3 h-3 text-white" fill="none" viewBox="0 0 20 20">
                    <polyline points="5,10 9,14 16,7" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </div>
                记住我
              </label>
        ...
        <div
          class="w-1/2 flex flex-col items-center justify-center px-10 z-10 transition-all duration-700 ease-in-out"
          :class="isRegisterMode ? 'translate-x-0 opacity-60 scale-95' : '-translate-x-4 opacity-100 scale-100'"
        >
          <form class="space-y-4 w-full max-w-sm" @submit.prevent="handleRegister">
            ...
            <div class="space-y-3">
              ...
              <div class="flex gap-6 justify-center py-2">
                <label class="flex items-center cursor-pointer text-slate-600">
                  <input
                    ...
                    class="sr-only"
                  />
                  <div class="w-5 h-5 mr-2 border-2 border-slate-300 rounded-full flex items-center justify-center transition-all duration-200"
                       :class="{ 'border-blue-500': registerForm.gender === 'male' }">
                    <div v-if="registerForm.gender === 'male'" class="w-2.5 h-2.5 bg-blue-500 rounded-full"></div>
                  </div>
                  男
                </label>
                <label class="flex items-center cursor-pointer text-slate-600">
                  ...
                  <div class="w-5 h-5 mr-2 border-2 border-slate-300 rounded-full flex items-center justify-center transition-all duration-200"
                    ...
                    <div v-if="registerForm.gender === 'female'" class="w-2.5 h-2.5 bg-blue-500 rounded-full"></div>
                  </div>
                  女
                </label>
              </div>
              ...
              <div class="flex items-center text-xs text-slate-500 mt-1">
                <span class="mr-1">
                  <!-- 优化后的灯泡svg -->
                  <svg width="16" height="16" fill="none" viewBox="0 0 20 20" class="inline align-middle">
                    <ellipse cx="10" cy="8" rx="6" ry="6" stroke="#6366f1" stroke-width="1.5"/>
                    <path d="M8 17h4m-2 1v-1" stroke="#3b82f6" stroke-width="1.5" stroke-linecap="round"/>
                  </svg>
                </span>
                仅支持学术邮箱
              </div>
              ...
              <div class="relative">
                ...
                <span class="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none">
                  <!-- 优化后的学校svg -->
                  <svg width="22" height="22" fill="none" viewBox="0 0 24 24" class="opacity-70">
                    <polygon points="12,4 21,10 12,16 3,10" fill="none" stroke="#6366f1" stroke-width="2"/>
                    <rect x="5" y="14" width="14" height="4" rx="2" stroke="#3b82f6" stroke-width="2"/>
                  </svg>
                </span>
              </div>
              <div class="relative">
            ...
            <div class="flex items-center py-2">
              <label class="flex items-center cursor-pointer text-slate-600 text-sm">
                ...
                <div class="w-4 h-4 mr-2 border-2 border-slate-300 rounded flex items-center justify-center transition-all duration-200"
                  ...
                  <svg v-if="registerForm.agreeTerms" class="w-3 h-3 text-white" fill="none" viewBox="0 0 20 20">
                    <polyline points="5,10 9,14 16,7" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </div>
                我同意
                <button type="button" @click="showServiceTerms = true" class="text-blue-500 hover:text-blue-600 underline mx-1">服务条款</button>
                和
                <button type="button" @click="showPrivacyPolicy = true" class="text-blue-500 hover:text-blue-600 underline mx-1">隐私政策</button>
              </label>
            </div>
            <button
              type="submit"
        ...
        >
          <div class="text-center px-8">
            <template v-if="!isRegisterMode">
              ...
              <p class="text-blue-100 leading-relaxed mb-8">使用您的账户登录，继续您的学术探索之旅</p>
              <button
              ...
              >
                ...
                <div class="absolute inset-0 bg-white/10 rounded-full scale-0 group-active:scale-100 transition-transform duration-200"></div>
              </button>
            </template>
            <template v-else>
              <h3 class="text-2xl font-semibold mb-4">新用户？</h3>
              <p class="text-blue-100 leading-relaxed mb-8">注册账户加入我们的学术社区，与全球研究者建立联系</p>
              <button
                class="px-8 py-3 bg-white/20 backdrop-blur-sm border-2 border-white/30 rounded-full text-white font-medium hover:bg-white/30 hover:border-white/50 transform hover:-translate-y-1 transition-all duration-200 shadow-lg hover:shadow-xl relative overflow-hidden group"
                @click="switchMode"
              >
          ...
          <!-- 遮罩装饰 -->
          <div class="absolute inset-0 pointer-events-none">
            <div class="absolute w-48 h-48 border-2 border-white/10 rounded-full -top-12 -left-12 animate-spin-slow"></div>
            <div class="absolute w-32 h-32 border-2 border-white/10 rounded-full -bottom-8 -right-8 animate-spin-reverse"></div>
            <div class="absolute w-24 h-24 border-2 border-white/10 rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 animate-pulse"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 服务条款模态框 -->
    <div v-if="showServiceTerms" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" @click="showServiceTerms = false">
      <div class="bg-white rounded-2xl p-8 max-w-2xl max-h-[80vh] overflow-y-auto m-4" @click.stop>
        <div class="flex justify-between items-center mb-6">
          ...
          <button @click="showServiceTerms = false" class="text-slate-400 hover:text-slate-600">
        ...
        <div class="space-y-4 text-slate-600 leading-relaxed">
          ...
          <p>欢迎来到 KnoWeb！我们是一群热爱学术的"码农"，立志打造一个让全世界学者都能愉快交流的平台。如果你发现Bug，请温柔一点告诉我们，我们的程序员心理很脆弱的。😅</p>
          ...
          <p>请妥善保管你的账号密码，不要设置成"123456"或者"password"。如果你的密码被破解了，我们会很无奈地摇头叹息的。</p>
          
          <h4 class="text-lg font-semibold text-slate-800">📊 第四条：数据使用</h4>
          <p>我们承诺不会把你的学术成果拿去申请专利，也不会偷偷看你的研究笔记。你的数据安全对我们来说比程序员的头发还重要！</p>
          
          <h4 class="text-lg font-semibold text-slate-800">⚠️ 第五条：免责声明</h4>
          <p>如果因为使用本平台导致你熬夜写论文、沉迷学术无法自拔、或者突然对研究产生了过度热情，我们概不负责。学术有风险，研究需谨慎！</p>
          
          <h4 class="text-lg font-semibold text-slate-800">🔄 第六条：条款更新</h4>
          <p>我们保留随时修改这些条款的权利，但承诺不会改得太离谱。如有更新，我们会通过站内消息、邮件或者托梦的方式通知你。</p>
          
          <p class="text-sm text-slate-500 mt-6">最后更新：2025年6月25日（程序员加班的第3天）</p>
        </div>
        <div class="flex justify-end mt-6">
          <button @click="showServiceTerms = false" class="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
            我已阅读并理解（虽然有点搞笑）
          </button>
        </div>
      </div>
    ...
    <!-- 隐私政策模态框 -->
    <div v-if="showPrivacyPolicy" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" @click="showPrivacyPolicy = false">
      <div class="bg-white rounded-2xl p-8 max-w-2xl max-h-[80vh] overflow-y-auto m-4" @click.stop>
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-2xl font-bold text-slate-800">KnoWeb 隐私政策</h3>
          <button @click="showPrivacyPolicy = false" class="text-slate-400 hover:text-slate-600">
            <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
              <path d="M18 6L6 18M6 6l12 12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
            </svg>
          </button>
        ...
        <div class="space-y-4 text-slate-600 leading-relaxed">
          ...
          <p>我们会收集你的基本信息（姓名、邮箱、所属机构），就像图书馆办借书证一样正常。我们不会偷看你的浏览历史，也不会关心你是否在凌晨3点还在刷论文。</p>
          ...
          <p>你的数据就像我们程序员的私藏代码一样珍贵，我们会用最高级别的加密技术保护它们。连我们的CEO都看不到你的聊天记录（除非你们聊的是如何优化我们的平台）。</p>
          
          <h4 class="text-lg font-semibold text-slate-800">🍪 第四条：Cookie政策</h4>
          <p>我们使用Cookies来记住你的偏好，就像你最喜欢的咖啡店记住你的常规订单一样。这些Cookies不含卡路里，请放心食用。</p>
          
          <h4 class="text-lg font-semibold text-slate-800">🗑️ 第五条：数据删除</h4>
          <p>如果你要删除账号，我们会按照你的要求删除相关数据。但请三思，毕竟重新建立学术网络比重新安装微信还麻烦。</p>
          
          <h4 class="text-lg font-semibold text-slate-800">👮 第六条：数据共享</h4>
          <p>我们不会将你的数据卖给广告商、不会泄露给你的导师、更不会告诉你妈妈你几点睡觉。你的学术隐私，我们守护！</p>
          
          <h4 class="text-lg font-semibold text-slate-800">📞 第七条：联系我们</h4>
          <p>如果你对隐私政策有任何疑问，可以随时联系我们。我们的客服比论文审稿人回复还要快！</p>
          
          <p class="text-sm text-slate-500 mt-6">最后更新：2025年6月25日（隐私保护比WiFi密码还严密）</p>
        </div>
        <div class="flex justify-end mt-6">
          <button @click="showPrivacyPolicy = false" class="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
            明白了，你们还挺有趣的
          </button>
        </div>
      </div>
...
@keyframes float-4 {
  ...
  66% { transform: translateY(30px) rotate(-180deg); }
}

.animate-float-1 { animation: float-1 12s infinite ease-in-out; }
.animate-float-2 { animation: float-2 14s infinite ease-in-out 2s; }
.animate-float-3 { animation: float-3 10s infinite ease-in-out 4s; }
.animate-float-4 { animation: float-4 16s infinite ease-in-out 1s; }
