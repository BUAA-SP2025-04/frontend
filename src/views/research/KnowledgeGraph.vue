    ...
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <header class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            <!-- 视图切换 -->
            <div class="flex items-center bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
              <button
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div class="p-5 space-y-5">
                <!-- 图谱类型选择 -->
                <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">图谱类型</label>
                ...
                <!-- 布局类型 -->
                <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">布局类型</label>
                  <select 
                    v-model="layoutType"
                    @change="updateLayout"
                ...
                <!-- 筛选器 -->
                <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">按机构筛选</label>
                  <select 
                ...
                <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">按研究领域筛选</label>
                  <div class="relative">
                    <select 
                ...
                <!-- 搜索 -->
                <div>
                  <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">搜索研究人员</label>
                  <div class="relative">
                    <input 
                      v-model="searchQuery" 
                      type="text"
        ...
        <div class="lg:col-span-6">
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden h-full flex flex-col">
            <!-- 图谱头部 -->
            ...
            <div class="graph-container relative flex-1 min-h-[600px]">
              ...
              >
                <div class="text-center">
                  ...
                  <p class="mt-4 text-slate-600 dark:text-slate-400 font-medium">正在构建知识网络...</p>
                </div>
              </div>
              ...
              >
                <div class="text-center max-w-sm">
                  ...
                  <h3 class="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">暂无图谱数据</h3>
            ...
            <!-- 图谱工具栏 -->
            <div class="p-2 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
              <!-- 过滤控制 -->
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div v-if="selectedNode" class="p-5 space-y-5">
                ...
                <div class="flex items-center space-x-4">
                  <div class="relative">
                    <div class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 flex items-center justify-center overflow-hidden">
                      <img
                      ...
                      />
                      <span v-else class="text-2xl font-bold text-blue-500">{{ selectedNode.initials }}</span>
                    </div>
                    
                    <div v-if="selectedNode.type" class="absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center" :class="getNodeTypeClass(selectedNode.type).bg">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p v-if="selectedNode.institution" class="text-sm text-slate-500 dark:text-slate-400">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/40 dark:to-cyan-900/40 rounded-xl p-4 flex flex-col items-center justify-center border border-blue-100 dark:border-blue-800/50">
                  ...
                  <div class="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/40 dark:to-teal-900/40 rounded-xl p-4 flex flex-col items-center justify-center border border-emerald-100 dark:border-emerald-800/50">
                    <span class="text-2xl font-bold bg-gradient-to-r from-emerald-500 to-teal-500 bg-clip-text text-transparent">
                ...
                <div v-if="selectedNode.researchFields && selectedNode.researchFields.length">
                  <h5 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">研究领域</h5>
                ...
                <div v-if="selectedNode.bio">
                  <h5 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">简介</h5>
                ...
                <!-- 关联节点 -->
                <div v-if="relatedNodes.length" class="pt-4 border-t border-slate-200 dark:border-slate-700">
                  <h5 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">关联节点</h5>
                  <div class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                    ...
                    >
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3" :class="getNodeTypeClass(node.type).bg">
                        <i :class="[getNodeTypeClass(node.type).icon]"></i>
                      </div>
                      
                      <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                          {{ node.name }}
                        </p>
              ...
              <div v-else class="p-10 flex flex-col items-center justify-center text-center">
                <div class="mb-4 w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                  <i class="fas fa-hand-pointer text-2xl text-slate-400 dark:text-slate-500"></i>
                </div>
                <h4 class="font-medium text-slate-700 dark:text-slate-300 mb-1">选择节点查看详情</h4>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div class="p-5 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                ...
                <button class="text-slate-400 hover:text-blue-500 dark:text-slate-500 dark:hover:text-blue-400 transition-colors">
              ...
              <div class="p-5 space-y-4">
                <!-- 影响力排行 -->
                <div>
                  <h5 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3 flex items-center">
                  ...
                  <div class="space-y-2">
                    <div 
                      ...
                      class="flex items-center p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700/50 cursor-pointer group"
                    >
                      <div class="mr-3 w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs" :class="getInfluenceRankClass(index)">
                        {{ index + 1 }}
                      </div>
                      
                      <div class="flex-1 flex items-center min-w-0">
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                          {{ user.name }}
                        </span>
                        <div class="ml-2 px-1.5 py-0.5 rounded text-xs bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400">
                          {{ user.score }}
                        </div>
                      </div>
                    </div>
                ...
                <div class="pt-4 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 text-center">
                      <div class="text-lg font-semibold text-slate-700 dark:text-slate-200">{{ networkMetrics.avgDegree }}</div>
                      <div class="text-xs text-slate-500 dark:text-slate-400">平均连接度</div>
                    </div>
                    
                    <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 text-center">
                      <div class="text-lg font-semibold text-slate-700 dark:text-slate-200">{{ networkMetrics.clusterCoef }}</div>
                      <div class="text-xs text-slate-500 dark:text-slate-400">聚类系数</div>
                    </div>
                    
                    <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 text-center">
                      <div class="text-lg font-semibold text-slate-700 dark:text-slate-200">{{ networkMetrics.communities }}</div>
                      <div class="text-xs text-slate-500 dark:text-slate-400">社区数量</div>
                    </div>
                    
                    <div class="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 text-center">
                      <div class="text-lg font-semibold text-slate-700 dark:text-slate-200">{{ networkMetrics.diameter }}</div>
...
const mockNodes = [
  // 研究人员节点
  { id: 'p1', name: '张教授', type: 'person', institution: '清华大学', title: '教授', researchFields: ['人工智能', '机器学习', '深度学习'], metrics: { publications: 156, followers: 2340 }, bio: '人工智能领域专家，专注于深度学习算法优化和应用研究。', avatar: '' },
  { id: 'p2', name: '李研究员', type: 'person', institution: '北京大学', title: '副教授', researchFields: ['自然语言处理', '机器学习'], metrics: { publications: 89, followers: 1580 }, bio: '计算语言学专家，研究方向为自然语言处理与知识图谱构建。', avatar: '' },
  { id: 'p3', name: '王博士', type: 'person', institution: '中国科学院', title: '研究员', researchFields: ['计算机视觉', '图像处理'], metrics: { publications: 67, followers: 890 }, bio: '计算机视觉与图像处理专家，专注于智能医疗影像分析。', avatar: '' },
  { id: 'p4', name: '陈教授', type: 'person', institution: '复旦大学', title: '教授', researchFields: ['数据挖掘', '大数据分析'], metrics: { publications: 203, followers: 3450 }, bio: '数据科学领军人物，在大数据挖掘与分析领域有深入研究。', avatar: '' },
  { id: 'p5', name: '刘博士', type: 'person', institution: '浙江大学', title: '副教授', researchFields: ['人工智能', '强化学习'], metrics: { publications: 78, followers: 1200 }, bio: '强化学习与决策智能领域研究者，关注AI在复杂系统中的应用。', avatar: '' },
  { id: 'p6', name: '赵研究员', type: 'person', institution: '中国科学院', title: '助理研究员', researchFields: ['机器学习', '图神经网络'], metrics: { publications: 45, followers: 620 }, bio: '图神经网络与知识表示学习研究者。', avatar: '' },
  { id: 'p7', name: '孙教授', type: 'person', institution: '清华大学', title: '教授', researchFields: ['人机交互', '认知计算'], metrics: { publications: 112, followers: 1850 }, bio: '人机交互专家，研究智能系统与人类认知的交互模式。', avatar: '' },
  { id: 'p8', name: '钱副教授', type: 'person', institution: '北京大学', title: '副教授', researchFields: ['计算语言学', '自然语言处理'], metrics: { publications: 67, followers: 930 }, bio: '计算语言学专家，专注于多语言自然语言理解。', avatar: '' },
  { id: 'p9', name: '周研究员', type: 'person', institution: '中国科学院', title: '研究员', researchFields: ['生物信息学', '机器学习'], metrics: { publications: 89, followers: 1240 }, bio: '生物信息学与机器学习交叉领域的研究者，研究计算生物学模型。', avatar: '' },
  { id: 'p10', name: '吴教授', type: 'person', institution: '浙江大学', title: '教授', researchFields: ['人工智能', '计算机视觉'], metrics: { publications: 132, followers: 2150 }, bio: '视觉计算与图像理解专家，计算机视觉领域国际知名学者。', avatar: '' },
...
const loadGraphData = async () => {
  ...
  try {
    ...
    if (selectedInstitution.value) {
      ...
      // 找出与所选机构相关的节点
      filteredLinks
        .filter(link => link.source === selectedInstitution.value || link.target === selectedInstitution.value)
        .forEach(link => {
    ...
    if (selectedResearchField.value) {
      ...
      // 找出与所选研究领域相关的节点
      filteredLinks
        .filter(link => link.source === selectedResearchField.value || link.target === selectedResearchField.value)
        .forEach(link => {
          relatedNodeIds.add(link.source);
          relatedNodeIds.add(link.target);
        });
    ...
    if (graphType.value === 'follow') {
      ...
      // 保留机构节点和与机构直接相关的节点
      const institutionIds = new Set(filteredNodes.filter(node => node.type === 'institution').map(node => node.id));
      
      filteredLinks = filteredLinks.filter(link => 
        (institutionIds.has(link.source) || institutionIds.has(link.target)) ||
      ...
      // 保留研究领域节点和相关连接
      const fieldIds = new Set(filteredNodes.filter(node => node.type === 'field').map(node => node.id));
      
      filteredLinks = filteredLinks.filter(link => 
        (fieldIds.has(link.source) || fieldIds.has(link.target)) ||
    ...
    if (searchQuery.value.trim()) {
      ...
      filteredNodes = filteredNodes.filter(node => 
        ...
        (node.type === 'person' && (node as any).researchFields?.some((field: string) => field.toLowerCase().includes(query)))
      );
...
const getNodeTypeClass = (type: string) => {
  const classes = {
    person: { bg: 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400', icon: 'fas fa-user' },
    institution: { bg: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400', icon: 'fas fa-university' },
    field: { bg: 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400', icon: 'fas fa-book' }
  };
